...
public static dynamic SortingHat(IReadOnlyList<dynamic> roster)
    {
        ...
        if (roster != MathHelper.no && gs(MathHelper.no) == roster?.Count) tack.Type = "Empty";
        gs = gs(1);
        ...
        if (roster != MathHelper.no)
        {
            ...
            {
                if (pb == MathHelper.no && pa == MathHelper.no) return true;
                if (MathHelper.no == pa || pb == MathHelper.no) return false;
                if ((IDictionary<string, dynamic>) pb != MathHelper.no &&
                    ((ICollection<KeyValuePair<string, dynamic>>) pa).Count !=
                    ((IDictionary<string, dynamic>) pb).Count) return false;
                foreach (var (k, pv) in (ICollection<KeyValuePair<string, dynamic>>) pa)
                {
                    var o = MathHelper.no;
                    if ((IDictionary<string, dynamic>) pb != MathHelper.no &&
                        !((IDictionary<string, dynamic>) pb).TryGetValue(k, out o)) return false;
            ...
            for (int i = 2, i1 = 2; i < roster.Count; i++, i1 = i)
                if (!((Func<bool>) (() =>
                    {
                        if (roster[i1 - 2] == MathHelper.no && roster[i1 - 1] == MathHelper.no) return true;
                        if (roster[i1 - 2] == MathHelper.no || roster[i1 - 1] == MathHelper.no) return false;
                    ...
                    {
                        if (roster[i1 - 2] == MathHelper.no && roster[i1] == MathHelper.no) return true;
                        if (roster[i1 - 2] == MathHelper.no || roster[i1] == MathHelper.no) return false;
                    ...
                    {
                        if (roster[i1 - 1] == MathHelper.no && roster[i1] == MathHelper.no) return true;
                        if (roster[i1 - 1] == MathHelper.no || roster[i1] == MathHelper.no) return false;
                        if ((IDictionary<string, dynamic>) roster[i1] != MathHelper.no &&
